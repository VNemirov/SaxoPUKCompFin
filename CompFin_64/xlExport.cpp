	...
	LPXLOPER12	gridTech)
{
			...
			LPXLOPER12	gridTech)
	{
		...
		if (numRows > 0 && !kXlUtils::getDbl(contract, 0, 0, expiry, &err))	return kXlUtils::setError(err);
		if (numRows > 1 && !kXlUtils::getDbl(contract, 1, 0, strike, &err))	return kXlUtils::setError(err);
		if (numRows > 1 && !kXlUtils::getInt(contract, 2, 0, dig, &err))	return kXlUtils::setError(err);
		if (numRows > 2 && !kXlUtils::getInt(contract, 3, 0, pc, &err))		return kXlUtils::setError(err);
		...
		if (pSetting == 0) {
			...
			for (i = 0; i < s.size(); i++)
			{
				...
				for (int j = 0; j < numT; j++) {
					...
					kXlUtils::setDbl(i + 3, j + 1, res(i, j), out);
				}
			}
		}
		else {
			//	size output
			numRows = 3 + 2 * (max(0, numS) / 2 + 1);
			numCols = numT + 1;
			...
			for (i = 0; i < 2 * (max(0, numS) / 2 + 1); i++)
			{
				...
				for (int j = 0; j < numT; j++) {
					...
					kXlUtils::setDbl(i + 3, j + 1, pMatrix(i, j), out);
				}
			}
		}
...
extern "C" __declspec(dllexport) int xlAutoOpen(void)
{
	...
	Excel12f(xlfRegister, 0, 11, (LPXLOPER12)&xDLL,
		...
		(LPXLOPER12)TempStr12(L"QQQQ"),
		(LPXLOPER12)TempStr12(L"xBlackFwdRunner"),
		(LPXLOPER12)TempStr12(L"params, contract, gridTech"),
		(LPXLOPER12)TempStr12(L"1"),
		(LPXLOPER12)TempStr12(L"myOwnCppFunctions"),
		(LPXLOPER12)TempStr12(L""),
